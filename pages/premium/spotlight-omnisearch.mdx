...
```js
Hooks.on("spotlightOmnisearch.indexBuilding", (promises) => {
...
`spotlightOmnisearch.indexBuilding` is called before `spotlightOmnisearch.indexBuilt` and it's used to add promises to the promises array that will be awaited before calling `spotlightOmnisearch.indexBuilt` hook.
...
```js
    const BaseSearchTerm = CONFIG.SpotlightOmniseach.SearchTerm;
