...
Hooks.on("spotlightOmnisearch.indexBuilt", (INDEX, promises) => {
  const mySearchTerms = await getMySearchTerms();
  INDEX.push(...mySearchTerms);
  promises.push(...myPromises);
});
```

If the process to create your searchterms is async, you can push a promise to the promises array and it will be awaited before the search is available.
