...
Power Select Toolkit will add 10 Keybindings (by default `Shift + 0-9`) that will execute a macro which includes `SuperSelectN` in the name, where `N` is the number of the keybinding. You can use the Export to Macro feature of the selection tool to create these macros.