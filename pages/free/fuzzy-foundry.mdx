...
- You can use the `&&` and the `||` operators to search for multiple
  properties at once. For example, `&zombie&&multiattack` will search for
  zombies with a multiattack action. `&zombie||skeleton` will search for
