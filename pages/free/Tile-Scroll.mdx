...
You can recognize this problem by the error `Canvas drawing failed:
this.mesh.setShaderClass is not a function`
...
Tile scroll will not work with tiles with an `Occlusion Mode` of
`Radial` or `Vision`
