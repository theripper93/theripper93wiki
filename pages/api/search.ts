...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (req.method === "GET") {
        ...
        try {
            const results = [];
            if (title) {
                const titleSearchResults = doSearch(title) ?? [];
                const titleResults = titleSearchResults.map((r) => {
                    return {
			...
			if (query) {
				const querySearchResults = doSearch(query) ?? [];
                const queryResults = querySearchResults.map((r) => {
                    return {
                        route: "https://wiki.theripper93.com" + r.route,
                        title: r.title,
                        content: shortenString(r.content, contentLength),
