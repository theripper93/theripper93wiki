...
Create new ```Script Macros``` in your game with the ``` blocks [3D
...
````@dev7355608``` for tons of help and suggestions on many parts of the 3D Canvas developement`  
````@Otigon#2010``` for the wonderful Automated Animations integration ([httpswww.patreon.comotigon Patreon])`  
````@arcanist#4317``` for testing and ``` tips ([httpswww.patreon.comarcanistzed Patreon])`
