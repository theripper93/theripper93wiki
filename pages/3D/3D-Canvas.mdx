...
<Callout type='error' emoji='️🚫'>
  ...
  Particle3D insde the .onEnd() can have an .onEnd() as well, allowing for
  infinite chaining.
</Callout>
...
<Callout type='info' emoji='ℹ️'>
  The .arc() parameter will make each projectile curve in a different direction!
</Callout>
...
<Callout type='info' emoji='ℹ️'>
  The .to() is omitted from the .onEnd() explosion effect so that the explosion
  plays on the target hit.
</Callout>
...
- A playlist of 3D Canvas tutorials.
...
#### Importing Maps from Townscaper by Chryron
