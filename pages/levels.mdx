...
elevation and sight is based on tile transparency. Note that the image itself needs to have transparent areas. Use your preferred software to export transparent images or cut out the areas afterwards with software like GIMP.
...
been deprecated from V10 onwards in favour of tile transparency. While before you needed BOTH a `hole` drawing and a transparent tile to see through, now you only need a transparent tile.
...
<Callout type='info' emoji='ℹ️'>
  Remember that the Background layer is a solid plane that will block vision even with transparency [read more](#background-elevation).
</Callout>
...
4. Open the Levels UI (also known as the _Levels Layer Tool_) by
   pressing the <FontAwesomeIcon icon={faLayerGroup} /> button.

5. Create the levels in the UI as you did in the [Your First Levels Map](#your-first-levels-map) guide. You should have 3 levels, one for each floor.
...
9. If you have any `Roofs` you will need to manually set them up in the same way you did in the [Your First Levels Map](#your-first-levels-map) guide.
...
It's highly recommended to make a backup of your scene before doing this.
If you have a scene with existing objects that you want to set to a specific level, follow these steps:

<div className='steps-container-ol'>
1. Select all the objects you want to set to a level.
2. Delete them.
3. Open the Levels UI (also known as the _Levels Layer Tool_) by
   pressing the <FontAwesomeIcon icon={faLayerGroup} /> button.
...
It seems that the Foundry wall optimization algorithm is not working properly when the scene has a lot of stacked walls.
Unfortunately this issue can only be fixed by core and cannot be fixed by Levels.
