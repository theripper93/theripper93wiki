...
<Tabs items={['Dependencies', 'Recommended', 'Submodules']}>
  <Tab>
    <>
      ...
      [libWrapper](https://github.com/ruipin/fvtt-lib-wrapper) - [Wall
      Height](Wall-Height)
    </>
  </Tab>
  <Tab>
  ...
  <Tab>
    These modules are developed by theripper93 himself that extend **Levels**'
    features. - [Automatic Cover Calculator](Levels-Auto-Cover) - [Depth
    Blur](Levels-Depth-Blur) - [Volumetric
    Templates](Levels-Volumetric-Templates)
  </Tab>
</Tabs>
...
In this example the lighting looks correct at first glance. `Light Masking` can
be toggled in the `Lighting` tab in the scene settings. By enabling this feature
all lights below the current point of view will be visible and masked with the tiles that are above them.
...
If you are **not** using the **[Perfect Vision](https://foundryvtt.com/packages/perfect-vision)** 
module you will encounter the limitations listed below:
